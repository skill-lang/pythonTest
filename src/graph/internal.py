...
class SkillState(State):
    ...
    def __init__(self, poolByName, strings, annotationType, types, inStream, mode, knownTypes):
        ...
        try:
            ...
            self.Node = p if (p is not None) else Parser.newPool("node", None, types, self._knownTypes[0])
        except Exception as e:
...
class P0(BasePool):
    ...
    def __init__(self, poolIndex, cls):
        ...
        """
        super(P0, self).__init__(poolIndex, "node", ["color", "edges"], [None for i in range(0, 0)], cls)
    ...
    def addKnownField(self, name, string, annotation):
        if name == "color":
            ...
            F1(SetType(self.owner().Node), self)
    ...
    def make(self, color=None, edges=None):
        """
        :return a new Node instance with the argument field values
        """
        rval = self._cls(-1, color, edges)
        self.add(rval)
        return rval
...
class F0(KnownDataField):
    ...
    def __init__(self, fType, owner):
        super(F0, self).__init__(fType, "color", owner)
...
class F1(KnownDataField):
    ...
    def __init__(self, fType, owner):
        ...
        if False:  # TODO type check!:
            raise SkillException("Expected field type set<node> in Node.edges but found {}".format(fType))
